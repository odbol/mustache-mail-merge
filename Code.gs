...
function selectDraftInGmail_(app, scheduled) {
  ...
  var ccPanel = app.createHorizontalPanel().setVerticalAlignment(UiApp.VerticalAlignment.MIDDLE).setSpacing(10);
  var ccAddr = UserProperties.getProperty('mustacheGlobalCC');
  var ccAddrBox = app.createTextBox().setName('ccAddr').setWidth(200).setText(ccAddr);
  ccPanel.add(app.createLabel('CC:')).add(ccAddrBox);
  parametersPanel.add(ccPanel);
